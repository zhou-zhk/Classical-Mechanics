...
\textbf{Corollary} $\frac{\mathrm{d}H}{\mathrm{d}t} = \frac{\partial H}{\partial t}$: If a system whose $hamiltonian$ function satisfies $\frac{\partial H}{\partial t}=0$, which means does not depend explicitly on time, the laws of conservation holds: $H(\mathrm{p}(t), \mathrm{q}(t)=const$.
